...
void AddImageToLoad(DrImage** load_to, std::string image_file, bool outline) {
    ImageData img_data { };
        img_data.image =        load_to;
        img_data.image_file =   image_file;
        img_data.outline =      outline;
    ...
    sfetch_request_t sokol_fetch_image { };
        ...
        sokol_fetch_image.callback = +[](const sfetch_response_t* response) {
            ...
            if (bmp.isValid()) {
                ...
                (*(l_image_load_stack[0].image)) = new DrImage(l_image_load_stack[0].image_file, bmp, l_image_load_stack[0].outline);
